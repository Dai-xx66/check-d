...
Phase 1 基础架构、Phase 2 任务核心、Phase 3 计时系统、Phase 4 日历与 Phase 5 标签统计：
...
- 日/周/月/年时间分布、标签占比、长期任务完成率和目标达成次数
- 各任务累计成功天数、当前连续和历史最长连续天数
- 任务详情展示长期目标剩余天数及周/月/年完成率，日历当天详情展示标签时间
- 依据任务修改历史还原旧周期，未来日期不进入本期完成率分母

公开网页当前保持旧版本；后续 UI 重新设计并确认后再发布更新。

计划和复盘功能将按照后续阶段逐步实现。节假日数据源、通知、云端同步执行器尚未启用；已有排除记录可参与统计过滤。当前浏览器数据按设备本地时区划分，固定账号时区及跨时区迁移留待同步阶段。
