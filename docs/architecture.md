...
- 长期任务和单次事项共用同一套计时状态机；只有目标计时任务要求目标时长。
- v5 迁移保留已有完成历史，旧目标计时模式迁移为目标时长计时；迁移后的新记录必须手动确认完成。
...
Phase 3 已加入可靠计时状态机、分段 Session、每日时长聚合、跨日拆分和实时进度。长期任务支持自由计时、目标时长计时和点击直接完成，单次事项支持普通与计时模式。计时只在状态变化时持久化，运行中显示由 `started_at` 与当前时间推导；同一用户只允许一个 Session 处于 `running`。
...
- 长期任务完成率只计算截至今天的应执行日；TaskCompletion.exclusionReason 非空的日期排除。通过 TaskRevision 的前后快照恢复历史执行周期。归档/暂停从次日停止产生应执行记录。
